...
import { validateEntityExist, validateFileExist } from './book.valid.db.js'
...
export const validateUpdatedBook = async (currentBook, updatedBook) => {
  ...
  if ((!currentBook.idPublisher) || currentBook.idPublisher === undefined || currentBook.idPublisher === null) {
    currentBook.idPublisher = updatedBook.fkIdPublisher
  }
  if ((!currentBook.idAuthor) || currentBook.idAuthor === undefined || currentBook.idAuthor === null) {
    currentBook.idAuthor = updatedBook.fkIdAuthor
  }
  if (currentBook.isEnabled === undefined || currentBook.isEnabled === null) {
    currentBook.isEnabled = updatedBook.isEnabled
  }
  // Validacion basica
  validateIsEnabled(currentBook.isEnabled)
...
const validateIsEnabled = (isEnabled) => {
  if (typeof isEnabled !== 'boolean') {
    throw new BadRequestError400('El campo \'isEnabled\' solo puede ser true o false.')
  }
}
